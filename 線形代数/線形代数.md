...
\sum_{k=1}^{n}c_ia_i=0
$$
を満たす$c_i$が$c_i=0$しか存在しない場合、ベクトルの集合$a_i$が __線形独立__ という。あるベクトル空間の任意の元が __線形独立__ ベクトルの集合 $\{v_k\}_{k=1}^{i=n}$ で表せるとき、ベクトルの集合$\{v_k\}_{k=1}^{i=n}$を  __基底と呼び、次元はnとなる。__ 
...
\bold{T_ix_i=0}
$$
を解くことで固有ベクトルが求まるわけだがここで、__自由度j__ ならば固有ベクトルがj個求まるので対角化可能。なお、自由度は$\bold{A}$をn次正方行列とすると、
$$
n-rank(\bold{T_i})
$$
と求まる。
...
$R^n$の正規直交基底$\{\bold{u_1、u_2、\cdots 、u_n}\}$を列ベクトル成分にもつ行列
$$
...
計量線形空間$\bold{R^n}$の正規直交でない基底$\bold{\{a_1、\cdots 、a_n\}}$は正規直交基底$\bold{\{u_1、\cdots 、u_m、\cdots 、u_n\}}$に変換可能。具体的にはm=1の時は単純に
$$
...
$^t\bold{A=A}$なる行列$\bold{A}$は対称行列と呼ばれ、この行列は対角化可能である。また、固有ベクトル$\bold{x_i、x_j}$は __それぞれ直交する。__ つまり、行列$\bold{A}$は(正規)直交行列$\bold{U=[x_1、x_2、\cdots、x_n]}$を用いて、$\bold{U^{-1}AU}$で対角化可能。
...
* 足りない分だけ$\bold{T_ix_i'=X_i、T_ix_i''=X_i'}$等を計算していって、固有ベクトルを作る。なお、この操作で固有ベクトルを作ることで各固有ベクトルは線形独立となる。つまりPに逆行列の存在が保証される。(証明P236)
